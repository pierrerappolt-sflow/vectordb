...
def create_search_app(index_manager: VectorIndexManager, database_url: str) -> FastAPI:
    ...
    async def batch_index_vectors(request: BatchIndexRequest) -> BatchIndexResponse:
        ...
        try:
            # Lookup VectorizationConfig to get dimensions, strategy, and metric
            config_row = await pool.fetchrow(
                """
                SELECT dimensions, vector_indexing_strategy, similarity_metric
            ...
            dimensions = config_row["dimensions"]
            strategy = config_row["vector_indexing_strategy"]
            similarity_metric_str = config_row["similarity_metric"]

            # Convert similarity metric string to enum
            try:
                similarity_metric = VectorSimilarityMetric(similarity_metric_str)
            except ValueError as e:
                msg = f"Invalid similarity metric '{similarity_metric_str}' for config {request.config_id}"
