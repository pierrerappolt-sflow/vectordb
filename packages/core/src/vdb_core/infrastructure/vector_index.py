...
class VectorIndex:
    ...
    def search(self, query_vector: list[float], k: int) -> tuple[list[str], list[float]]:
        ...
        if self.metric == VectorSimilarityMetric.L2:
            # L2 distance: ||a - b||^2 (Euclidean distance)
            # Lower is better
            scores = np.linalg.norm(self.vectors - query_array, axis=1)
            # Sort ascending (smaller distance = more similar)
            sorted_indices = np.argsort(scores)

        elif self.metric == VectorSimilarityMetric.L1:
            # L1 distance: sum(|a - b|) (Manhattan distance)
            # Lower is better
            scores = np.sum(np.abs(self.vectors - query_array), axis=1)
            # Sort ascending (smaller distance = more similar)
            sorted_indices = np.argsort(scores)
            ...
            # Sort descending (higher similarity = more similar)
            sorted_indices = np.argsort(scores)[::-1]

        elif self.metric == VectorSimilarityMetric.DOT_PRODUCT:
            # Dot product: a · b
            scores = np.dot(self.vectors, query_array)
            # Sort descending (higher dot product = more similar)
            sorted_indices = np.argsort(scores)[::-1]
    ...
    def remove(self, embedding_id: str) -> bool:
        ...
        # Rebuild ID mapping (indices shifted after removal)
            ...
            while True:
                rows = await pool.fetch(
                    ...
                    SELECT e.id, e.library_id, e.vectorization_config_id, e.vector, e.dimensions,
                           vc.vector_indexing_strategy, vc.similarity_metric
                    FROM embeddings e
